...
export function FilterDropdown({
  ...
  return (
    <div ref={ref} className={cn("relative", className)}>
      <button
        ...
        className={cn(
          ...
          value !== "__all__"
            ...
            : "bg-base-200 border-base-300 text-base-content/50 hover:text-base-content",
        )}
      >
        <span>{label}</span>
        {value !== "__all__" && (
          <>
            ...
            <span className="font-semibold text-base-content max-w-[120px] truncate">
              {displayLabel}
            </span>
            <button
              type="button"
      ...
      {open && (
        <div className="absolute top-full left-0 mt-1 z-50 min-w-[200px] rounded-xl border bg-base-100 text-base-content shadow-lg overflow-hidden animate-modal-expand"
            ...
            <div className="p-2 border-b">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="搜索..."
                  className="w-full h-8 pl-8 pr-3 text-xs rounded-md border border-base-300 bg-base-200 outline-none focus:ring-1 focus:ring-primary"
                  autoFocus
                />
              </div>
            </div>
            ...
            {filtered.map((option) => (
              ...
              >
                ...
                {option.count !== undefined && (
                  <span className="tabular-nums text-base-content/50">{option.count}</span>
                )}
              </button>
            ))}

            {filtered.length === 0 && (
              <p className="px-2.5 py-3 text-xs text-base-content/50 text-center">
                无匹配项
              </p>
            )}
          </div>
